...
public class MakeshiftWingsFeatureRenderer<T extends LivingEntity, M extends EntityModel<T>> extends FeatureRenderer<T, M> {
    ...
    private final MakeshiftWingsEntityModel<T> makeshiftWings;

    public MakeshiftWingsFeatureRenderer(FeatureRendererContext<T, M> context, EntityModelLoader loader) {
        super(context);
        this.makeshiftWings = new MakeshiftWingsEntityModel<>(loader.getModelPart(GraysModEntityModelLayers.MAKESHIFT_WINGS));
    }
    ...
    public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T livingEntity, float f, float g, float h, float j, float k, float l) {
        ...
        if (itemStack.isOf(GraysModItems.MAKESHIFT_WINGS)) {
            ...
            this.getContextModel().copyStateTo(this.makeshiftWings);
            this.makeshiftWings.setAngles(livingEntity, f, g, j, k ,l);
            VertexConsumer vertexConsumer = ItemRenderer.getArmorGlintConsumer(vertexConsumerProvider, RenderLayer.getArmorCutoutNoCull(WINGS_TEXTURE), itemStack.hasGlint());
            this.makeshiftWings.render(matrixStack, vertexConsumer, i, OverlayTexture.DEFAULT_UV);
            matrixStack.pop();
        }
    }
}
